...
public class KesfetAnlıkDusuncelerAdaptor extends RecyclerView.Adapter<KesfetAnlıkDusuncelerAdaptor.AnlıkDusuncelerHolder> {
    ...
    ConstraintLayout kesfetAnlikDusuncelerView;


    public KesfetAnlıkDusuncelerAdaptor(String arananParca,ArrayList<KesfetAnlıkDusuncelerDetails> kesfetAnlıkDusuncelerDetailsArrayList,Context context,ConstraintLayout kesfetAnlikDusuncelerView) {
        this.arananParca = arananParca;
        this.kesfetAnlıkDusuncelerDetailsArrayList = kesfetAnlıkDusuncelerDetailsArrayList;
        this.context = context;
        this.kesfetAnlikDusuncelerView = kesfetAnlikDusuncelerView;
    ...
    public void onBindViewHolder(@NonNull AnlıkDusuncelerHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        String puan = kesfetAnlıkDusuncelerDetailsArrayList.get(position).getPuan();
        ...
        Timestamp timestamp = kesfetAnlıkDusuncelerDetailsArrayList.get(position).getTarih(); // Firebase'den gelen Timestamp değeri
        ...
        YorumSayisiniGör(holder,position);
        begen(kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderiId(),holder.begenImage,holder.begenText);
        begeniSayisi(kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderiId(),holder.begeniSayisiText);
            ...
            public void onDilCallback(String dil) {
             ...
             try {
                 ...
                 if (dil.equals("türkce"))
                {
                    ...
                    {
                        String parcaModeli = kesfetAnlıkDusuncelerDetailsArrayList.get(position).getParcaModeli();
                        ...
                        if (parcaModeli.equals("Memoirs (Memoirs)")) {
                            ...
                            holder.parcaModeli.setText(kesfetAnlıkDusuncelerDetailsArrayList.get(position).getParcaModeli());
                        }
        ...
        if(gonderiId != null){
            ...
            holder.yorumyapLinear.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    kesfetAnlikDusuncelerView.setVisibility(View.GONE);
                    NavDirections action = HomeFragmentBottomNavDirections.actionHomeFragmentBottomNavToYorumlarFragment(gonderiId)
                            .setFromWhere("anlik_düsünce")
                            .setArananParca(arananParca)
                            .setGelenId(kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderenId());
                    Navigation.findNavController(view).navigate(action);
            ...
            public void onClick(View v) {
                ...
                if(holder.begenText.getText().equals("Beğen") || holder.begenText.getText().equals("Like")){
                    FirebaseDatabase.getInstance().getReference().child("AnlikDusunce_Begeniler")
                            .child(kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderiId()).child(userId).setValue(true);



                    String gonderiId = kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderiId();
                    ...
                    FirebaseDatabase.getInstance().getReference().child("AnlikDusunce_Begeniler")
                            .child(kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderiId()).child(userId).removeValue();
    ...
    private void YorumSayisiniGör(AnlıkDusuncelerHolder holder, int position){

        dilTanı(new DilCallback() {
            @Override
            public void onDilCallback(String dil) {

                if(!kesfetAnlıkDusuncelerDetailsArrayList.isEmpty()){
                    if (!kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderiId().isEmpty()){
                        DatabaseReference databaseReference2 = FirebaseDatabase.getInstance().getReference("Yorumlar").child(kesfetAnlıkDusuncelerDetailsArrayList.get(position).getGonderiId()).child("Gonderiler");
